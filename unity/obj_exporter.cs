...
public class OBJExporter : ScriptableWizard
{
    ...
    void Export(string exportPath)
    {
        ...
        if (generateMaterials)
        {
            System.IO.File.WriteAllText(System.IO.Path.Join(exportFileInfo.Directory.FullName, baseFileName + ".mtl"), sbMaterials.ToString());
        }
    ...
    string ExportTexture(Texture2D t)
    {
        try
        {
            ...
            string exportName = System.IO.Path.Join(lastExportFolder, t.name + ".png");
            ...
            return System.IO.Path.GetFileName(exportName);
        }
